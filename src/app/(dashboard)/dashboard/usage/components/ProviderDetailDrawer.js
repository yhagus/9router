        ...
        <tbody>
          {rows.map((row) => {
            ...
            return (
              ...
              >
                <td className="py-2 px-2 font-mono text-xs truncate max-w-[200px]" title={row.rawModel}>
                  {row.rawModel}
                </td>
...
// Shared skeleton for the by-account and by-API-key sections. `label` is
// resolved server-side; `id` is the raw connectionId / key string.
function BreakdownTable({ rows, header, emptyMessage }) {
  if (!rows?.length) {
    return <p className="text-sm text-text-muted py-4 text-center">{emptyMessage}</p>;
  }
  return (
    <div className="overflow-x-auto -mx-1">
      <table className="w-full min-w-[360px] text-sm text-left">
        <thead>
          <tr className="text-[11px] uppercase tracking-wide text-text-muted border-b border-black/5 dark:border-white/5">
            <th className="py-2 px-2 font-medium">{header}</th>
            <th className="py-2 px-2 font-medium text-right">Req</th>
            <th className="py-2 px-2 font-medium text-right">Tokens</th>
            <th className="py-2 px-2 font-medium text-right">In / Out</th>
          </tr>
          ...
          {rows.map((row) => {
            ...
            return (
              <tr
                ...
                className="border-b border-black/[0.03] dark:border-white/[0.03] last:border-b-0"
              >
                <td className="py-2 px-2 font-medium truncate max-w-[220px]" title={row.id}>
                  {row.label || row.id || "—"}
                </td>
...
export default function ProviderDetailDrawer({
  isOpen,
  onClose,
  loading,
  providerMeta,
  ...
  return (
    <Drawer
      isOpen={isOpen}
      onClose={onClose}
      title={providerMeta?.name || "Provider"}
      width="xl"
    >
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-center gap-2 text-xs text-text-muted">
            ...
            {providerMeta?.id && providerMeta.id !== providerMeta.name && (
              <span className="font-mono">{providerMeta.id}</span>
            )}
          </div>
          ...
          <section>
            ...
            <BreakdownTable
              rows={stats.byAccount}
              header="Account"
              emptyMessage="No account usage in this period"
            />
          </section>
